...
void _applyAppName(String root, String appName, String appNameEn, String packageId) {
  ...
  // 3e. lib/common/constant.dart — appNameEn (兼容单引号和双引号)
  count += _replaceInFile(
    p.join(root, 'lib', 'common', 'constant.dart'),
    RegExp("const appNameEn\\s*=\\s*['\"][^'\"]*['\"]"),
  ...
  // 也处理 common 模块的 strings.xml
  count += _replaceInFile(
    p.join(root, 'android', 'common', 'src', 'main', 'res', 'values', 'strings.xml'),
    RegExp(r'<string name="FlClash">[^<]*</string>'),
    '<string name="FlClash">$appName</string>',
    'common/strings.xml',
  );

  // 5b. Android: VPN 和通知相关字符串
  count += _replaceInFile(
    p.join(root, 'android', 'service', 'src', 'main', 'java', 'com', 'follow', 'clash', 'service', 'VpnService.kt'),
    RegExp(r'setSession\("[^"]*"\)'),
    'setSession("$appName")',
    'VpnService.kt (session)',
  );
  count += _replaceInFile(
    p.join(root, 'android', 'service', 'src', 'main', 'java', 'com', 'follow', 'clash', 'service', 'modules', 'NotificationModule.kt'),
    RegExp(r'setContentTitle\("[^"]*"\)'),
    'setContentTitle("$appName")',
    'NotificationModule.kt (title)',
  );
  count += _replaceInFile(
    p.join(root, 'android', 'service', 'src', 'main', 'java', 'com', 'follow', 'clash', 'service', 'models', 'NotificationParams.kt'),
    RegExp(r'val title: String = "[^"]*"'),
    'val title: String = "$appName"',
    'NotificationParams.kt (title)',
  );
  count += _replaceInFile(
    p.join(root, 'android', 'app', 'src', 'main', 'kotlin', 'com', 'follow', 'clash', 'models', 'State.kt'),
    RegExp(r'currentProfileName: String = "[^"]*"'),
    'currentProfileName: String = "$appName"',
    'State.kt (profileName)',
  );
  count += _replaceInFile(
    p.join(root, 'android', 'service', 'src', 'main', 'java', 'com', 'follow', 'clash', 'service', 'FilesProvider.kt'),
    RegExp(r'add\(DocumentsContract\.Root\.COLUMN_TITLE, "[^"]*"\)'),
    'add(DocumentsContract.Root.COLUMN_TITLE, "$appName")',
    'FilesProvider.kt (title)',
  );
  ...
  // 6b. Android: google-services.json — 更新包名以匹配 applicationId
  final gsPath = p.join(root, 'android', 'app', 'google-services.json');
  count += _replaceInFileAll(
    gsPath,
    RegExp(r'"package_name":\s*"com\.follow\.clash(\.dev|\.debug)?"'),
    '"package_name": "$packageId"',
    'google-services.json (package_name)',
  );
  // Also handle already-migrated package names from previous branding runs
  count += _replaceInFileAll(
    gsPath,
    RegExp(r'"package_name":\s*"[^"]*"'),
    '"package_name": "$packageId"',
    'google-services.json (all package_name)',
  );
  ...
  // 12. lib/common/constant.dart — appName (兼容单引号和双引号)
  count += _replaceInFile(
    p.join(root, 'lib', 'common', 'constant.dart'),
    RegExp("const appName\\s*=\\s*['\"][^'\"]*['\"]"),
...
void _applyIcons(String root, String iconDirRel) {
  ...
  // 检查是否有预制的 Android 特定图标 (android/ 或 android_webp/ 目录)
  for (final subDir in ['android', 'android_webp']) {
    final androidIconDir = Directory(p.join(iconDir.path, subDir));
    if (!androidIconDir.existsSync()) continue;
    for (final entry in androidDpis.entries) {
      for (final name in ['ic_launcher.webp', 'ic_launcher_round.webp', 'ic_launcher.png']) {
        final src = File(p.join(androidIconDir.path, entry.key, name));
        if (src.existsSync()) {
          count += _copyIcon(
  ...
  // ---- Android: Play Store 高清图标 ----
  final playstoreSrc = File(p.join(iconDir.path, 'android_webp', 'ic_launcher-playstore.png'));
  if (playstoreSrc.existsSync()) {
    count += _copyIcon(
        playstoreSrc.path, p.join(androidResDir, '..', '..', '..', 'ic_launcher-playstore.png'));
  }
